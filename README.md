...
Second, I ran multiple correlation heatmaps to see which variables correlated best with `all_nba_selections`. Since it's a binary column, there wasn't "great" correlation numbers. However, the highest were VORP, points, win shares and turnovers. Assists and rebounds were up there as well.

Finally, initial EDA showed that choice of college didn't make a huge difference. I created dummy columns for each college listed (over 370 in total) and none of them had a significant correlation with `all_nba_selections`. There were a couple exceptions where there was maybe one player from a college and that player happened to be great. However, no other players came out of that school and made it to the NBA. Therefore, I do not see any evidence indicating college has an impact.
